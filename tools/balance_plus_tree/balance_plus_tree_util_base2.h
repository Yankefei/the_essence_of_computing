#ifndef _TOOLS_BALANCE_PLUS_TREE_UTIL_BASE2_H_
#define _TOOLS_BALANCE_PLUS_TREE_UTIL_BASE2_H_
...
public:
    BalancePlusTree_Util2(int m) : m_(m), m_half_(m % 2 == 0 ? m/2 : m/2 + 1)
    {
    }

    BalancePlusTree_Util2(const BalancePlusTree_Util2& rhs)
    {
        copy(rhs);
    }

    void copy(const BalancePlusTree_Util2& rhs)
    {
        m_ = rhs.m_;
        m_half_ = rhs.m_half_;
    }
    ...
    */
    ...
    bool order_ele(Node* begin_ptr, size_t ele_size)
    {
        bool res = true;
        if (begin_ptr == nullptr) return res;
        ...
        res = (ele_num == ele_size ? true : false);
